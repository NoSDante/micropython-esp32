...
class SPS30:
  ...
  def __init__(self, port=1, rx=9, tx=10, debug=False, sample=60, interval=1200, raw=False, calibrate=None, addr=SPS_ADDR, clean=0, explicit=False):
    ...
    try:
      if type(port) is str: # no PyCom case
        ...
        self.ser = UART(port, baudrate=115200, bits=8, parity=None, stop=1, rx=rx, tx=tx)
