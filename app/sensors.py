...
class Sensors:
    ...
    def init_SPS30(self, port=1, rx=9, tx=10, start=True, clean=True, sample=60):
        ...
        try:
            self.sps30 = SPS30(port=UART(port, baudrate=115200, bits=8, parity=None, stop=1, rx=rx, tx=tx),
                               debug=self.debug, sample=sample)
        except Exception as e:
    ...
    def init_SCD30(self, i2c=None, start=True, auto_calibration=False, forced_co2=None, temp_offset=None,
                   meas_interval=None, pause=1000):
        self.scd30 = None
        ...
        # default 2          
        if meas_interval is not None and isinstance(meas_interval, int):
            if self.debug: print("set meas_interval {}second(s)".format(meas_interval))
            self.scd30.set_measurement_interval(meas_interval)
    ...
    def heatindex(self, temp, hum):
        ...
        if fahrenheit >= 80 and hum >= 40:
            ...
            C1 = [-42.379, 2.04901523, 10.14333127, -0.22475541, -6.83783e-03, -5.481717e-02, 1.22874e-03, 8.5282e-04,
                  -1.99e-06]
            C2 = [0.363445176, 0.988622465, 4.777114035, -0.114037667, -0.000850208, -0.020716198, 0.000687678,
                  0.000274954, 0]
            ...
            heatindex3 = C3[0] + (C3[1] * fahrenheit) + (C3[2] * hum) + (C3[3] * fahrenheit * hum) + (C3[4] * T2) + (
                        C3[5] * H2) + (C3[6] * T2 * hum) + (C3[7] * fahrenheit * H2) + (C3[8] * T2 * H2) + (
                                     C3[9] * T3) + (C3[10] * H3) + (C3[11] * T3 * hum) + (C3[12] * fahrenheit * H3) + (
                                     C3[13] * T3 * H2) + (C3[14] * T2 * H3) + (C3[15] * T3 * H3)
